...
func (utils UtilsService) EncryptPlainText(plaintText string) (string, error) {
	block, err := aes.NewCipher([]byte(config.EncryptionKey))
	if err != nil {
		return "", err
	}

	text := []byte(plaintText)
...
func (utils UtilsService) DecryptEncryptedText(encryptedPlainText string) (string, error) {
	cipherText, err := base64.StdEncoding.DecodeString(encryptedPlainText)
	if err != nil {
		return "", err
	}
